...
impl Normalize for DMatrix<f64> {
    ...
    fn normalize_spotis(
        ...
        let expected_solution_point =
            DVector::from_fn(num_criteria, |i, _| match criteria_types[i] {
                CriterionType::Cost => bounds.row(i).min(),
                CriterionType::Profit => bounds.row(i).max(),
            });
